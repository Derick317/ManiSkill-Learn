...
def main_mfrl_brl(cfg, args, rollout, evaluator, logger):
    ...
    from mani_skill_learn.apis.train_rl_replan import train_rl
    ...
    if not args.evaluation:
        ...
        replay_train_onPol = build_replay(cfg.replay_train_onPol_cfg)
        replay_val_rand = build_replay(cfg.replay_train_rand_cfg)
        replay_val_onPol = build_replay(cfg.replay_train_onPol_cfg)
        buffers = {
            "trainOnPol": replay_train_onPol, 
            "trainRand": replay_train_rand,
            "valOnPol": replay_val_onPol, 
            "valRand": replay_val_rand
        }
        train_rl(agent, rollout, evaluator, cfg.env_cfg, buffers, work_dir=cfg.work_dir, eval_cfg=cfg.eval_cfg,
                 **cfg.train_mbrl_cfg)
    else:
